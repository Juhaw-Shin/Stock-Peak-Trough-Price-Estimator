...
def current_constituents() -> pd.DataFrame:
    ...
    cache = NEW / "SP500_Constituents.csv"
    try:
        frame = pd.read_csv(url)
        atomic_text(cache, frame.to_csv(index=False))
    except Exception:
        if not cache.exists():
            raise
...
def collect_constituents(frame: pd.DataFrame, end_date: str, benchmark_dates: np.ndarray, refresh: bool) -> dict[str, str]:
    CACHE.mkdir(parents=True, exist_ok=True)
    seeded = seed_trained_tickers(benchmark_dates) if not refresh else set()
    status: dict[str, str] = {}
    pending: list[str] = []
    for ticker in frame["YahooSymbol"]:
        ...
        if ticker in seeded:
            ...
            if len(saved["price"]) >= MIN_HISTORY_DAYS and lag_days <= 7:
...
def current_feature(
    ...
    trough = int(extrema[-1])
    if trough not in troughs:
        return None, {"reason": "latest_confirmed_extremum_is_peak"}
...
def predict_all_sp500(end_date: str, refresh: bool = False, top: int = 25) -> pd.DataFrame:
    ...
    result = pd.DataFrame(rows).sort_values(
        ["lower_remaining_upside_pct", "expected_remaining_upside_pct"], ascending=False,
    ).reset_index(drop=True)
    result.insert(0, "rank", np.arange(1, len(result) + 1))
    excluded_frame = pd.DataFrame(excluded)
    atomic_text(NEW / "Stock_Predictions.csv", result.to_csv(index=False))
    atomic_text(NEW / "Stock_Predictions_Top.json", result.head(top).to_json(orient="records", indent=2))
    atomic_text(NEW / "Stock_Prediction_Excluded.csv", excluded_frame.to_csv(index=False))
    summary = {
        ...
        "ranking": "lower-bound remaining upside from current price",
    ...
    atomic_text(NEW / "Stock_Prediction_Summary.json", json.dumps(summary, indent=2))
    ...
    if len(result):
        columns = [
            "rank", "ticker", "current_price", "lower_peak_price", "expected_peak_price",
            "upper_peak_price", "lower_remaining_upside_pct", "duration_days", "reliability",
        ]
...
def main() -> None:
    ...
    try:
        predict_all_sp500(end_date=end_date, refresh=args.refresh, top=args.top)
