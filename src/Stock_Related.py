...
Training_Source = np.load("NPZ/Training Data/Training_Source.npy")
Target = np.load("NPZ/Training Data/Target.npy")
...
def Random_Smooth(batch_size):
    x = rng.integers(0, 933561, batch_size)
    return [Training_Source[x], Target[x][:, np.newaxis]]

def Random_Smooth_O(batch_size):
