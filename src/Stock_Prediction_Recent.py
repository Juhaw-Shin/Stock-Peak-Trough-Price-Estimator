...
RECENT_OUTPUT_PREFIX = "Stock_Predictions_Recent"
...
def predict_all_sp500(
    ...
    atomic_text(NEW / f"{RECENT_OUTPUT_PREFIX}.csv", result.to_csv(index=False))
    atomic_text(NEW / f"{RECENT_OUTPUT_PREFIX}_Top.json", result.head(top).to_json(orient="records", indent=2))
    atomic_text(NEW / f"{RECENT_OUTPUT_PREFIX}_Excluded.csv", excluded_frame.to_csv(index=False))
    summary = {
    ...
    atomic_text(NEW / f"{RECENT_OUTPUT_PREFIX}_Summary.json", json.dumps(summary, indent=2))
    ...
    if len(result):
        columns = [
            "rank", "ticker", "data_date", "current_price", "lower_peak_price", "expected_peak_price",
