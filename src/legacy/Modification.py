...
"""
def Cost_D(expect, result):
    if (result > 0 and expect <= result) or (result <= 0 and expect >= result):
        return 2 * (expect - result)
    return 3 * (expect - result)
"""
def ReLU(x):
...
def Part_Rolling_Back(From_To, Part_X_Currunt, Part_X_Next, Part_Weight_Next, Part_PDLX_Next):
    ...
    return [Part_PDLX_Prior, Learning_Rate * Part_Weight_Grad, Learning_Rate * Part_Bias_Grad]
